...
class UserAdminProcess @Inject() (private  val repFactory: IUserRepositoryFactory, private val repType: RepTypes) extends IUserAdminProcess{
  ...
  override def deleteUsers(): Future[Boolean] = {
    ...
    for{
      ...
      //println(s"Найден пользователь ${user} с ролью ${user.get.ro.role.name}")
